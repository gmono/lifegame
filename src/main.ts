...
import { delay, float, int, randint, range, str } from "../libs/lib";
...
function create<T extends keyof table, R extends keyof table[T]>(
  tag: T,
  id: string,
  values: object
): table[T] {
...
async function main() {
  ...
  let init = () =>
    tf
      .randomUniform(size, 0, 1, defaultDtype)
      .div(float(getval("rel")))
      .floor()
      .equal(0)
      .asType(defaultDtype) as tf.Tensor2D;
  let dt = init();
  //输出
  ...
  async function loop() {
    ...
    for (;;) {
      ...
      if (!sl && n % drawFreq == 0) await d.draw2D(dt);
      //
      if (p) break;
      n++;
      //显示轮
  ...
  get("ctx").onclick = (e) => {
    if (e.button == 0) changepoint(e.offsetX, e.offsetY);
  };
